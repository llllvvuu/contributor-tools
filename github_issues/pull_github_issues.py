...
def get_issues(repo_full_name: str, max_issues: int, days: int) -> List[dict]:
    ...
    params = {'state': 'open', 'sort': 'updated', 'direction': 'desc', 'per_page': 100, 'since': cutoff_date.isoformat()}
    ...
    while url and len(all_issues) < max_issues:
        ...
        for issue in response.json():
            ...
            if len(all_issues) >= max_issues:
                break
        url = get_next_page_link(response.headers.get('Link', '')) if len(all_issues) < max_issues else None
    return all_issues

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--max_issues', type=int, default=30, help='Max issues per repo')
    parser.add_argument('--days', type=int, default=7, help='Look at issues from the last X days')
    ...
    for repo in repos:
        ...
        with open(args.output, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=['Repository', 'Issue URL', 'Issue Title', 'Issue Body', 'Created At', 'Updated At', 'Labels', 'Total Reactions'])
            writer.writeheader()
